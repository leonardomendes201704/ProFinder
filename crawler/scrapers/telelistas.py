...
class TelelistasScraper(BaseScraper):
    site_key = "telelistas"
    lead_source_name = "Telelistas"
    ...
    def seed_tasks(self, city: str, service: str, settings: CrawlerSettings) -> list[CrawlTask]:
        ...
        url = f"https://www.telelistas.net/busca?q={quote_plus(search_query)}"
    ...
    def _parse_html(self, html: str, task: CrawlTask, settings: CrawlerSettings) -> ScrapeResult:
        ...
        cards = soup.select("article, .resultado-item, .list-card, .card")
        providers: list[Provider] = []

        for card in cards:
            name_node = card.select_one("h2, h3, .title, .result-title, a[title]")
            ...
            website = next(
                (
                    href
                    for href in hrefs
                    if href and href.startswith("http") and "telelistas" not in href.lower()
                ),
                None,
            )
            detail_url = next(
                (urljoin(task.url, href) for href in hrefs if href and "/empresa/" in href.lower()),
                task.url,
            )
            ...
            providers.append(
                Provider(
                    ...
                    source_details_url=detail_url,
                    search_query=str(task.metadata["search_query"]),
                    raw_payload={"html_excerpt": text[:1000], "url": detail_url},
                )
            )
